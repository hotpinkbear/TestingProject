...
ID_CASES = [
    ...
    pytest.param("te1", "영문소문자 또는 숫자 4~16자로 입력해 주세요.", id = "TC008"),
    pytest.param("testab12345678900", "영문소문자 또는 숫자 4~16자로 입력해 주세요.", id = "TC009"),
    pytest.param("", "아이디를 입력해 주세요.", id = "TC010")
]

PW_CASES = [
    # pytest.param("test12#$", "test12#$", "회원가입이 완료 되었습니다.", id = "TC011"),
    pytest.param("TESTPW01234567890", "TESTPW01234567890", "비밀번호 입력 조건을 다시 한번 확인해주세요.", id = "TC012"),
    ...
    pytest.param("aaaaaaaaa", "aaaaaaaaa", "비밀번호 입력 조건을 다시 한번 확인해주세요.", id = "TC016"),
...
def test_pw_chk_val(driver_function, userpw, userpwcfm, expected):
    page = SignupPage(driver_function)
    page.open()
